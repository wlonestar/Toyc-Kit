...
//     left = std::make_unique<ImplicitCastExpr>(left->getType(),
//     std::move(left)); right =
//         std::make_unique<ImplicitCastExpr>(right->getType(),
//         std::move(right));
//     return left->getType();
//   }
...
//   if (match(LP)) {
//     auto func =
//         std::make_unique<DeclRefExpr>(dynamic_cast<DeclRefExpr
//         *>(expr.get()));
...
//       right =
//           std::make_unique<ImplicitCastExpr>(expr->getType(),
//           std::move(right));
//     }
...
//   clearVarTable();
//   std::vector<std::unique_ptr<ParmVarDecl>> params =
//   parseFunctionParameters(); consume(RP, "expected parameter declarator");
...
//   }
//   FuncKind kind =
//       (isExtern ? EXTERN_FUNC : (body == nullptr ? DECLARATION :
//       DEFINITION));
